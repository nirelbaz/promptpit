...
import { readManifest, writeManifest, computeHash, computeSkillHash } from "../core/manifest.js";
...
function agentFileName(adapterId: string, name: string): string {
...
function ruleFileNames(adapterId: string, name: string): string[] {
  switch (adapterId) {
    case "cursor":
...
async function removeCheckedFile(
  filePath: string,
  manifestHash: string,
